...
void msgCallback(const std_msgs::Float32MultiArray::ConstPtr& msg) {
	...
	for(int i = 0; i < 3; ++i) {
		m_head[i] = msg->data[i+2];
		m_move[i] = msg->data[i+5];
	}
	cmd = (int)(msg->data[8]+0.5);
}
...
int main(int argc, char** argv)
{
	...
	while(ros::ok) {
		...
		if(m_flag) {
			char target[256];
			sprintf(target, "fswebcam -q -d /dev/video0 -r 1024x720 --jpeg 88 -D 1 -F 3 ~/Desktop/photo.jpg &", cnt++);
			system(target);
			sleep(2);
		...
		if(cmd == 1) {
			msgo.data[1] = m_head[0];
			msgo.data[2] = m_head[1];
			msgo.data[3] = m_head[2];
		} else if(cmd == 2) {
			msgo.data[1] = m_head[0];
			msgo.data[2] = m_head[1];
			msgo.data[3] = m_head[2];
